...
public record CardData(byte[] data, String uuid, boolean locked, int color) {
    ...
    public static CardData read(CompoundTag tag) {
        ...
        byte[] data = tag.contains("data", Tag.TAG_BYTE_ARRAY)
                ? tag.getByteArray("data")
                : tag.contains("data", Tag.TAG_STRING)
                ? tag.getString("data").getBytes(StandardCharsets.UTF_8)
                : new byte[0];
        return new CardData(data, uuid, tag.getBoolean("locked"),
