...
public class ModelEnergyTurret extends ModelBase {
	...
	private ModelRenderer rotorBase;
	private ModelRenderer rotorShaft1;
	private ModelRenderer rotorShaft2;
	private ModelRenderer gunBase;
	private ModelRenderer gunBarrell;
	private ModelRenderer gunEnd;
	...
	public void render(float scale, EnergyTurret turret){
		if(turret != null)
			render(scale, turret.isUpright(), turret.getShaftLength(), turret.getBarrel(), turret.getRealYaw(), turret.getRealPitch());
		else
			render(scale, true, 1.0F, 1.0F, 0F, 0F);
	}
