...
public class UtilityGL11Debug {
    static HashMap<String, Boolean>lastFlags = new HashMap<>();

    public class GLproperty {
        GLproperty(int init_gLconstant, String init_name, String init_description, String init_category, String init_fetchCommand) {
    ...
    private GLproperty[] propertyList =
            {
    ...
    // resets the renderflags cache
    public static void reset(){
        lastFlags.clear();
    }

    // compares the current renderflags to the last call of the method
    public static void compareToLastFrame(){
        lastFlags = dumpEnabledToSetAndCompareTo(lastFlags);
    }

    // dumps the content of a set to system.out
    public static void dumpSetToConsole(HashMap<String, Boolean> set){
        for(Map.Entry<String, Boolean> entry : set.entrySet()){
            System.out.println(entry.getKey() + ": " + entry.getValue());
        }
    }

    // stores all flags and their state to a hashmap
    public static HashMap<String, Boolean> dumpEnabledToSet(){
        ...
        for (int i = 0; i < instance.propertyList.length; ++i)
            if (instance.propertyList[i].fetchCommand == "glIsEnabled()")
                list.put(instance.propertyList[i].name, GL11.glIsEnabled(instance.propertyList[i].gLconstant));

        return list;
    }

    // returns current flags and compares them to the oldSet (if the set has any entries)
    public static HashMap<String, Boolean> dumpEnabledToSetAndCompareTo(HashMap<String, Boolean> oldSet){
        if(oldSet.size() == 0)
            return dumpEnabledToSet();
        else {
            HashMap<String, Boolean> list = dumpEnabledToSet();
            dumpSetToConsole(compareSets(oldSet, list));
            return list;
        }
    }

    // compares two sets
    private static HashMap<String, Boolean> compareSets(HashMap<String, Boolean> a, HashMap<String, Boolean> b){
        HashMap<String, Boolean> list = new HashMap<>();
