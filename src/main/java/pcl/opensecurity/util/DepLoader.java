...
public class DepLoader implements IFMLLoadingPlugin, IFMLCallHook {
	...
	private static final String owner = "OpenSecurity DepLoader";
	...
	public static class DepLoadInst {
		...
		public DepLoadInst() {
			...
			v_modsDir = new File(mcDir, "mods/OpenSecurity/sounds/alarms");
			if (!v_modsDir.exists())
				v_modsDir.mkdirs();
		}
		...
		private String checkExisting(Dependency dep) {
			for (File f : v_modsDir.listFiles()) {
				...
				if (dep.file.filename.equals(f.getName())) {
		...
		private void load(Dependency dep) {
			...
			if (dep.existing == null)//download dep
			{
				...
				System.out.println("Skipping Dep, already exists");
			}
		}
		...
		private void scanDepInfo(File file) {
			if (!file.getName().endsWith(".jar") && !file.getName().endsWith(".zip")) {
				...
				try {
					...
					ZipEntry e = zip.getEntry("opensecdeps.info");
					if (e == null) e = zip.getEntry("opensecdeps.info");
					if (e != null)
						loadJSon(zip.getInputStream(e));
					zip.close();
				} catch (Exception e) {
					System.err.println("Failed to load opensecdeps.info from " + file.getName() + " as JSON");
					e.printStackTrace();
				}
			}
		}
		...
		private void addDep(Dependency newDep) {
			if (mergeNew(depMap.get(newDep.file.name), newDep)) {
				System.out.println("BLEH " + newDep.file.name + " Version " + newDep.file.version);
