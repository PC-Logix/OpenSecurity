...
public class ExtractDirectory {
	 ...
	 */
	public ExtractDirectory( URL sourceDirectory, String writeDirectory ) {
		// make sure write directory exists
		new File(writeDirectory).mkdirs();
		// extract into the write directory
		try {
			sourceDirectory = new URL("jar","",sourceDirectory.toString());
	 ...
	 * @throws IOException if an IO exception occurs
	 */
	private void extract( URL sourceDirectory, String writeDirectory ) throws IOException {
		//        final URL dirURL = getClass().getResource( sourceDirectory );
		//            sourceDirectory = new URL("jar",sourceDirectory.getPath(),"file:");

		final String path = sourceDirectory.getPath().substring(sourceDirectory.getPath().indexOf("!")).substring(2);
		if( ( sourceDirectory != null ) && sourceDirectory.getProtocol().equals( "jar" ) ) {
			final JarURLConnection jarConnection = (JarURLConnection) sourceDirectory.openConnection();
			...
			throw new IllegalStateException( "don't know how to handle extracting from " + sourceDirectory );
		}
	}
}